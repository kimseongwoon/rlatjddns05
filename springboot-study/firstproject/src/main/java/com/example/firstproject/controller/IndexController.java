...
public class IndexController {
    ...
    /**
     * Desc: 로그인한 유저이면 authentication에 유저에 대한 정보가 저장된 객체 만들어짐
    ...
    public String testLogin(
        ...
        if (authentication != null) {
            ...
            System.out.println("로그인한 유저 정보(authinfo): " + springSecPrincipalDetails.getUser());
    ...
    /**
     * Desc: 소셜로그인(OAuth2)한 정보 확인 용도 테스트
     * @param authentication
     * @return
     */
    @GetMapping("/test/oauth2/login")
    @ResponseBody
    public String testOAuth2Login(
            Authentication authentication,
            @AuthenticationPrincipal SpringSecPrincipalDetails userDetails
    ) {
        if (authentication != null) {
            ...
            System.out.println("소셜로그인한 유저 정보(oauthinfo): " + oAuth2User.getAttributes());
     ...
     * @return
     */
    @PostMapping("/join")     // return의 string의 값은 json형태로 받겠다.
