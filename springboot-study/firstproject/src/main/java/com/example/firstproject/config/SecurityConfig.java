...
//                                .failureUrl("/login-failure")
                ...
                // invalidateHttpSession은 로그아웃시 session을 초기화
                .logout(logout -> logout.logoutSuccessUrl("/loginForm").invalidateHttpSession(true))
                .oauth2Login(oauth2 ->
                        oauth2.loginPage("/loginForm")
                                .userInfoEndpoint(userInfoEndpoint -> userInfoEndpoint.userService(springSecOAuth2PrincipalDetailsService)))  // oauth2 인증시 필요
        ;
